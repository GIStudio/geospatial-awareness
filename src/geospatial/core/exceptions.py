...
class EdgeNotFoundError(GeospatialError):
    """当请求的边在图中不存在时抛出"""
...
class PathNotFoundError(GeospatialError):
    """当无法找到从起点到终点的路径时抛出"""
...
class DataFormatError(GeospatialError):
    """当输入数据格式不正确时抛出"""
...
class MatchingError(GeospatialError):
    """当坐标匹配失败时抛出"""
...
class VisualizationError(GeospatialError):
    """当可视化操作失败时抛出"""
...
class ConfigurationError(GeospatialError):
    """当系统配置错误时抛出"""
...
class ObserverError(GeospatialError):
    """当观察者操作失败时抛出"""
...
class AlgorithmError(GeospatialError):
    """当算法执行失败时抛出"""
...
class ValidationError(GeospatialError):
    """当数据验证失败时抛出"""
